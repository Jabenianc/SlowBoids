...
typedef std::map<std::string, std::vector<Boid*>> boid_hash;
typedef std::pair<std::string, std::vector<Boid*>> hash_pair;
typedef std::map<std::string, std::vector<Vector2*>> obs_hash;
typedef std::pair<std::string, std::vector<Vector2*>> obs_pair;
...
public:
	...
	float distance_to(Vector2 a_vector) const;

	// Returns a vector interpolated in between
	// this one and another.
	Vector2 linear_interpolate(Vector2 a_vector, float amount) const;
...
private:
	...
	static float _m_separate;
	static float _m_align;
	static float _m_cohede;
	static float _m_avoid;
