...
// Operator overloads
inline Vector2 Vector2::operator+(const Vector2& a_vector) {
...
float Vector2::distance_to(Vector2 a_vector) const {
	return (a_vector - *this).magnitude();
}

// Returns a vector that is an interpolation
// between vectors A and B.
Vector2 Vector2::linear_interpolate(Vector2 a_vector, float amount) const {
	float i = (x * (1 - amount)) + (a_vector.x * amount);
	float j = (y * (1 - amount)) + (a_vector.y * amount);
	return Vector2(i, j);
}
...
Vector2 Vector2::normalized() const {
	...
	float mag = magnitude();
	if (mag != 0) {
		Vector2 normalized(x / mag, y / mag);
		return normalized;
	} else {
		// If magnitude is 0, just return the vector itself.
		return *this;
	}
}
...
float Boid::_m_separate = 2.2;
float Boid::_m_align = 1.75;
float Boid::_m_cohede = 2.0;
...
Vector2 Boid::compute(boid_hash* p_hash, obs_hash* p_obs_hash) {
	...
	for (auto it = local_cell->begin(); it != local_cell->end(); it++)
	{
		Boid* p_boid = *it;
	...
	if (p_obs_hash->find(_m_cell) != p_obs_hash->end()) {
		...
		for (auto it = local_obs->begin(); it != local_obs->end(); it++)
		{
...
double Boid::get_rotation() const {
	double deg = 0.0;
	if (_m_dir.x != 0) {
		deg = atan(_m_dir.y/_m_dir.x) * 57.2958;
	}
	if (_m_dir.x < 0) {
		// Flip! += 180.0 works too!
		deg -= 180.0;
	}
	return deg;
}
...
void Boid::set_cell(std::string& cell) {
	_m_cell = cell;
}
...
const int Flightspace::_M_CELLSIZE = 25;
...
Flightspace::~Flightspace() {
	...
	for (auto boid = _mp_boids->begin(); boid != _mp_boids->end(); boid++) {
	...
	_mp_hash = nullptr;
	_mp_obs_hash = nullptr;
	_mp_boids = nullptr;
}

void Flightspace::random_populate(unsigned int size, int xmax,
	int ymax, float speed, float agility)
{
	for (int i = 0; i < size; i++) {
		Vector2 position(random<int>(0, xmax), random<int>(0, ymax));
		Vector2 dir(random<float>(-1, 1), random<float>(-1, 1));
		dir = dir.scaled(speed);
		Boid* new_boid = new Boid(position, dir, speed, agility);
